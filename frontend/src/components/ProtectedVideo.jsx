  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[300px] sm:min-h-[400px] gap-4 p-4 sm:p-8">
        <div className="w-8 h-8 sm:w-12 sm:h-12 border-4 border-accent border-t-transparent rounded-full animate-spin"></div>
        <p className="text-sm sm:text-lg text-secondary">Chargement...</p>
      </div>
    )
  }
  ...
  if (!isFirstVideo && (!isAuthenticated || !user)) {
    return (
      <div className="card-startup flex flex-col items-center justify-center min-h-[300px] sm:min-h-[400px] gap-4 sm:gap-6 p-4 sm:p-8 text-center my-6 sm:my-8">
        ...
        <p className="text-sm sm:text-lg max-w-md mb-4 sm:mb-6 px-4 text-secondary">
